...
> **当前 SDK 版本：v0.8.1** — 已覆盖知识树/章节/小节接口，HttpClient 支持自动路由

---

## 零、SDK v0.8.1 路由机制（✅ 已实现）

`HttpClient.resolveBaseURL(path)` 根据请求路径自动路由到对应服务：

| 路径前缀 | 路由到 | 配置地址 |
|----------|--------|----------|
| `/auth/*` | `services.auth` | `http://192.168.16.129:12301/api/v1` |
| `/knowledge/*` | `services.knowledge` | `http://192.168.16.129:12303/api/v1` |
| 其他 | `baseURL` | `http://192.168.16.129:12301/api/v1` |

> **注意**：下文"待补充接口"中以 `/knowledge/*` 开头的路径会自动路由到 12303 端口。
